...
namespace Emgu.CV
{
   ...
   public class EigenObjectRecognizer
   {
      ...
      {
        CvInvoke.cvEigenDecomposite(src.Ptr, Array.ConvertAll<Image<Gray, Single>, IntPtr>(eigenImages, delegate(Image<Gray, Single> img) { return img.Ptr; }), avg.Ptr);
      }
      #endregion
      ...
      public Image<Gray, Byte> EigenProjection(float[] eigenValue)
      {
         ...
         CvInvoke.cvEigenProjection(Array.ConvertAll<Image<Gray, Single>, IntPtr>(_eigenImages, delegate(Image<Gray, Single> img) { return img.Ptr; }),eigenValue, _avgImage.Ptr, res.Ptr);
         return res;
      }
      ...
      public float[] GetEigenDistances(Image<Gray, Byte> image)
      {
         using (Matrix<float> eigenValue = new Matrix<float>(EigenDecomposite(image, _eigenImages, _avgImage)))
            return Array.ConvertAll<Matrix<float>, float>(_eigenValues,
                ...
                {
                    return (float)CvInvoke.Norm(eigenValue.Ptr,Norm(eigenValue.Ptr, eigenValue.Ptr, Emgu.CV.CvEnum.NormType.L2, IntPtr.Zero,eigenValue.Ptr, eigenValueI.Ptr, Emgu.CV.CvEnum.NormType.L2/*.CvEnum.NORM_TYPE.CV_L2*/, IntPtr.Zero);//.cvNorm(eigenValue.Ptr, eigenValueI.Ptr, Emgu.CV.CvEnum.NORM_TYPE.CV_L2, IntPtr.Zero);
                });
      }
